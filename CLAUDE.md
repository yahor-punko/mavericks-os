...
- **Mirror rule** — every status change in `BACKLOG.md` must be mirrored in `TASK_STATUS.md` before the turn ends. `scripts/mavp-operator-sync-status.js` (run by the PostToolUse hook) also auto-creates a skeleton `TASK_STATUS.md` Active-tasks entry for any BACKLOG Active Wave task it doesn't yet find there — completing the mirror for creations, not just status updates — skipping `deprecated` tasks and any task carrying a real `Superseded by:` value.
