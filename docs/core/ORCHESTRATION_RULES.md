...
3. **The brief's `Test scope:` field** — a Main-Agent-seeded baseline (see `CLAUDE.md` — "Sub-agent brief template") that the developer extends via (1) and (2) above and reports back as a delta from what the Main Agent seeded. **Before seeding this baseline, the Main Agent must confirm the fragment substring-matches the basenames of the test files the task is expected to add or touch** — a filename-level check that requires no execution — and prefer the widest subsystem fragment over the most specific script name. Two incidents in one session were both over-narrowing, not over-widening: T-680 was seeded `--filter publish-release`; the wider `--filter publish` caught a sibling secret-scan interaction seconds after integration that the narrow fragment had missed. T-679 was seeded `--filter install`, a fragment that cannot substring-match `test-wrapper-*` — the task's OWN new test file — so that test never ran in the worktree, and canonical `main` sat CI-red across two consecutive pushes until the omission was found and fixed. The `install` vs `test-wrapper-*` case is the sharper failure mode: the seeded filter excluded the task's own new test, which is why the defect reached CI rather than being caught in the worktree.
