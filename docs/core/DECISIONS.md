...
## DR-013 — Per-operator memory is the residual codification route, decided by a portability test, and the routing mandate is not RCA-scoped

**Date:** 2026-08-23

**Informed by:** the 2026-08-23 T-708/T-709 instance — T-708 merged a false claim into a shipped normative doc (it characterized a code path as "a silent-degradation question" when the code prints a loud red WARNING and its own docstring says "never a silent skip"), T-709 was registered and merged to correct it, and the lesson drawn from it was written to per-operator memory (`feedback_verify_the_weakest_claim`) rather than into the framework; DR-009 (a rule the operator channel can silently violate must not stay advisory-only — cited here for its scope limit, which is what keeps this rule out of its reach).

**Tasks:** T-711 (this record and the tie-break it governs), T-712 (the booking-time claim-verification rule this ruling unblocks)

**Problem:** `docs/core/RCA_CODIFICATION.md` routes every root cause to exactly one of five durable mechanisms, but gave route (c) — a per-operator memory entry, outside the repo — only a *fit* test: "a recurring operational lesson worth surfacing across sessions, where the fix is 'remember this'." That test is unfalsifiable in practice, because every lesson is rememberable, so route (c) fits everything. It is also by far the cheapest route: one write, no task, no architect gate, no review. With no precedence rule between a route that fits everything and four routes that cost a full task cycle, the cheapest route silently wins every tie. It did exactly that on 2026-08-23, and nothing was violated when it happened, because no rule covered the path at all: the doc's mandate opens with "every **RCA document** must end in a Codification section," and this lesson came from a mid-session observation, not an RCA. The consequence of a framework-portable lesson landing in memory is that it is invisible to adopters, invisible to `docs/SKILL_OPTIMIZATION.md`'s reflection loop (which mines task outcomes, never memory), and not greppable over the repo — it dies with one operator's machine.

**What was considered:**
1. A full precedence ranking across all five routes, ordering (a)–(e) by strength so any tie between any two routes resolves mechanically.
2. Mechanical enforcement — a hook or validator check that detects a framework-portable lesson going to memory and blocks or flags it, per DR-009's escalation path.
3. Banning route (c) outright: remove memory from the routing table, so every codified lesson lands in the repo.
4. The adopted ruling: a single tie-break demoting route (c) to residual, decided by a portability test, plus extending the routing mandate to any durable lesson whether or not an RCA document exists.

**Why rejected (1, 2, 3):**
- **A full route ranking (1):** false precision. Routes (a), (b), (d) and (e) partition by the *shape* of the cause — a missing constraint, a sub-agent behavior pattern, a fix that cannot be verified yet, a checkpoint that is structurally too late — and they are not linearly ordered in strength for every cause; which one is strongest depends entirely on what the cause is, which is exactly what the existing "exactly one, best fit" language already asks the router to judge. Only route (c) competes with all four, because "remember this" fits everything, so one clause demoting (c) closes the whole observed defect class. Ranking the other four as well would over-specify a judgment the router has to make anyway, and over-specification invites rule-lawyering that erodes compliance with the part of the rule that actually bites.
- **Mechanical enforcement (2):** unavailable on both of DR-009's own terms. First, the write is unobservable from the repo — a memory entry lands outside the repo tree, where no PostToolUse hook fires and no validator run can read it; DR-009's scope limit says plainly that it governs "rules that govern the operator's own direct writes to **state artifacts**," naming `BACKLOG.md`, `TASK_STATUS.md`, and `PROCESS_STATE.json`, and this write is none of those. Second, even a hypothetical matcher with visibility could not judge *portability*, which is a semantic property of the lesson rather than a syntactic property of the string: the legitimate case (an operator-scoped lesson) and the violating case (a framework-portable one) are indistinguishable in form. DR-009's stated precondition for escalating any rule to a blocking tier is a matcher proven not to reject legitimate output — a precondition that cannot be met here even in principle, which is why this rule stays pull-consumed rather than sitting at an interim info tier waiting for an escalation that will never come.
- **Banning memory outright (3):** wrong in the other direction. The live memory index legitimately holds operator-personal entries — a language preference, this machine's git identity, this operator's permission posture — whose truth genuinely is scoped to one operator on one machine. A ban would push those into shipped, adopter-facing docs, polluting the framework with one person's preferences, which is a worse outcome than the one being fixed.

**What was adopted instead:**

A tie-break, not a ban and not a ranking. Route (c) becomes the **residual** route, chosen only when a portability test says no framework-level route applies. The test: *would this lesson be equally true for a different operator running this framework on a different machine?* **Yes** → framework-level; the cause must route to (a), (b), (d) or (e), and memory may hold **at most a pointer** naming the `T-NNN` or artifact that carries the rule. **No** → the lesson's truth is scoped to this operator or this machine, and memory is the correct and only sensible home.

The mandate is also un-scoped from RCA documents: the same routing table and the same tie-break govern **any** durable lesson, whether or not an RCA exists. The trigger to run the portability test is the mid-session urge to write a memory entry. A framework-level verdict means the Main Agent registers a task through the normal architect gate, exactly as for an RCA-sourced routing — a lesson being small is not a reason to skip the gate.

In place of mechanical enforcement, the rule carries a **compensating observable form**: a memory entry recording a framework-portable lesson must name inline the `T-NNN` or artifact carrying the rule, which makes compliance auditable by reading the memory index — a pointer with no carrier named is the visible failure. This is the same posture as the `Reopen trigger:` field and the registration-home convention in `CLAUDE.md`: deliberately pull-consumed, no validator check, relying on a form that makes non-compliance legible on inspection.

**Scope limit, stated plainly:** this decision adds **no validator check, no hook, and no new file**, and it changes nothing about routes (a), (b), (d), (e) or the "exactly one, best fit" language — only route (c)'s precedence and the mandate's scope. It does not make per-operator memory mandatory infrastructure: route (c) is optional, and an operator whose harness provides no per-operator memory loses nothing under this rule, because nothing load-bearing for the project may ever live *only* in memory. It also does not adjudicate the T-708 claim-verification lesson itself — routing that lesson into the framework is T-712's scope, and this record only establishes that it must be routed there rather than left in memory.

**Documented in:** `docs/core/DECISIONS.md` (this record); `docs/core/RCA_CODIFICATION.md` — route (c)'s row, "The portability tie-break", "Enforcement posture", and "Codification outside an RCA"; `templates/RCA_TEMPLATE.md` (Codification-section pointer); `CLAUDE.md` — "RCA-to-codification" convention (pointer); `docs/core/ORCHESTRATION_RULES.md` — "Booking-time claim verification".

## DR-014 — Budget awareness is spec-embedded per role, not a mandatory brief line; the roster is a frontmatter predicate; the recalibration evidence tier is accepted deliberately

**Date:** 2026-08-27

**Informed by:** the T-637 truncation and its own explicit ruling against a mechanical forcer for the `Turn budget:` brief field (`EXECUTION_LOG.md:640`); DR-009 (a rule the operator channel can silently miss must not stay advisory-only — the reasoning this decision distinguishes itself from, since a brief-line mandate is exactly the shape DR-009 warns against and cannot itself be escalated to, being unobservable); DR-013 (mechanical enforcement over memory whenever a framework-level route exists — the preference this decision follows in choosing a spec-embedded, test-enforced literal over a documented-but-unenforceable brief convention).

**Tasks:** T-727 (raised `qa` and `architect` `maxTurns` and recorded the recalibration evidence in `docs/TURN_BUDGET.md`), T-728 (added the `## Budget awareness` section to the six-role report-only roster and the `scripts/test-agent-spec-sync.js` assertions enforcing it), T-729 (this record, plus the canonical documentation home and the two convention tightenings it required)

**Problem:** Sub-agent reports were being truncated by turn-budget exhaustion, and because the report is written last, truncation ate exactly the deliverable — the report-only roles (roster defined below) have no other artifact a checkpoint commit could have protected. The obvious first fix — make the existing optional `Turn budget:` brief-line field mandatory on every spawn — was considered and refused, for reasons that needed a durable home rather than staying implicit in the refusal itself.

**What was considered:**
1. Make the `Turn budget:` brief-line field mandatory on every spawn to a report-only role.
2. Leave the field optional and rely on Main-Agent discipline (recon-preloading, scope narrowing) to keep report-only spawns under cap, as had been the practice before T-727/T-728.
3. The adopted design: embed the literal `maxTurns` number inside each report-only role's own spec file, in a machine-readable `` `maxTurns: N` `` form, with `scripts/test-agent-spec-sync.js` asserting both that the section exists (derived from a frontmatter `deny-tools` predicate, not a hand-maintained list) and that its number matches frontmatter — so the agent has a live number to self-count against without depending on the brief at all. The brief-line field survives as an override/retry channel, not the mechanism's only input.

**Why rejected (1, 2):**
- **Mandatory brief line (1):** unenforceable on the same grounds as completion-token checking — a brief is in-band chat content, not a file or artifact, so no hook, script, or validator can observe whether the field was actually filled. `docs/AGENT_SPEC.md` — "Report completion token" already states this limit for the token contract; a mandatory-but-unobservable rule for the turn-budget field is the identical shape. It had also already been ruled on once: `EXECUTION_LOG.md:640` records an explicit refusal of a mechanical forcer for this exact field, on this exact ground, following the T-637 cap-hit.
- **Status quo discipline (2):** this is what had already been operating at the time of the T-637 cap-hit and did not prevent it. Recon-preloading and scope-narrowing remain load-bearing brief-composition duties (`docs/core/ORCHESTRATION_RULES.md` — "Recon-preloading"), but neither gives the sub-agent itself a number to converge against mid-run — both are Main-Agent-side inputs at spawn time, not something the agent can check against its own progress.

**What was adopted instead:**

The roster boundary is a frontmatter predicate, not a judgment call: a role is in-roster when its `deny-tools:` line denies both `Edit` and `Write` — the report is its sole deliverable. The current roster (`analyst`, `architect`, `exa-researcher`, `qa`, `security-reviewer`, `ux`) is derived mechanically by `scripts/test-agent-spec-sync.js` and pinned with a `deepStrictEqual` against those six names, so a future report-only role fails the test until it grows the section rather than requiring a manual sweep to notice it needs one. `ui-designer` (capped at 20, but keeps `Write`) and the four committed-file roles (`developer`, `product-docs`, `technical-writer`, `frontend-design`) are out for the stated reason each: either the deliverable isn't the report, or a checkpoint commit already protects partial progress.

Each roster role's spec file carries a `## Budget awareness` section stating its own `` `maxTurns: N` `` literal plus a self-count instruction, an ~80% convergence trigger, a brief-line override note, and a role-tailored converge target. `docs/AGENT_SPEC.md` gains a canonical "Budget awareness" section following the completion-token pattern already established there: it is the single source of truth for the contract, while the operative text lives duplicated in each spec file because agent specs are standalone files the harness reads in isolation with no include mechanism — `scripts/test-agent-spec-sync.js` is what keeps the copies from drifting, not discipline. The `Turn budget:` brief-line description in both `docs/AGENT_SPEC.md` and `CLAUDE.md` is reworded, not removed: it is now framed as an override/retry channel for a spawn deviating from the role's own stated default, since the default itself no longer depends on the field being filled.

Two adjacent conventions were tightened as part of closing this out. First, `CLAUDE.md`'s per-spawn outcome convention now states explicitly that the `tool_uses: N/cap` denominator IS the role's frontmatter `maxTurns` at spawn time, and any other denominator must name its source inline — several recent `EXECUTION_LOG.md` entries carry a denominator matching neither the role's frontmatter value at the time nor a stated source (e.g. `developer` entries recorded against `/60` and `/50` while frontmatter read `140`, and a `qa` entry recorded against `/50` while frontmatter read `20`), and the artifacts alone cannot settle whether this reflects convention drift or a cap set outside the documented mechanism — this record states the rule and leaves the question open rather than asserting an explanation the evidence cannot support. Second, `docs/core/ORCHESTRATION_RULES.md` — "Cap-hit triage" now notes that a capped-resume completing with zero-or-near-zero new tool calls retroactively confirms the cap-hit diagnosis, since a resumed agent with genuine remaining work would consume calls doing it.

**Admissibility caveat — the evidence tier behind this mechanism's own numbers.** The `maxTurns` recalibration T-727 shipped for `qa` and `architect` rests substantially on a nine-run evidence table that `docs/TURN_BUDGET.md` itself labels operator-reported, from a project that consumes this framework rather than this repo — it is not transcript-confirmed here. Accepting that recalibration on this evidence tier was a deliberate ruling, not an oversight, made on the basis that it was corroborated locally: `docs/core/ORCHESTRATION_RULES.md` — "Capped-resume recovery protocol" records a local `qa` truncation at exactly its then-cap of 20 with no completion token, and the T-710 `EXECUTION_LOG.md` row (2026-08-23) records a second local `qa` report truncated at 20/20 that was resumed rather than re-diagnosed. Two independent local observations at the same cap value, on the same role, is corroboration of the direction of the external table's claim even though it cannot independently reproduce the external table's exact counts — the ruling accepts operator-reported evidence as admissible precisely when locally corroborated in this way, not as a general lowering of the evidence bar.

**Scope limit, stated plainly:** this decision changes no `maxTurns` value (T-727's scope) and adds no new roster member beyond the six T-728 already shipped — it records why the mandatory-brief-line alternative was refused, gives the mechanism a canonical documented home, and closes the denominator-source gap in the per-spawn outcome convention. It does not resolve which of the two explanations (convention drift vs. an out-of-band cap) produced the mismatched denominators found in `EXECUTION_LOG.md` — that stays an open question for a future session with more evidence.

**Documented in:** `docs/core/DECISIONS.md` (this record); `docs/AGENT_SPEC.md` — "Budget awareness" (canonical section) and the reworded `Turn budget:` brief-line description; `CLAUDE.md` — the reworded `Turn budget:` brief-line description and the tightened per-spawn outcome denominator rule; `docs/core/ORCHESTRATION_RULES.md` — "Cap-hit triage" (capped-resume confirmation sentence); `docs/TURN_BUDGET.md` (T-727's recalibration and its evidence-tier labeling, unmodified by this task); `.claude/agents/{analyst,architect,exa-researcher,qa,security-reviewer,ux}.md` (the six `## Budget awareness` sections, T-728, unmodified by this task); `EXECUTION_LOG.md:640` (the prior mandatory-brief-line refusal this record cites).
